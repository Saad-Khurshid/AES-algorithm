...
unsigned char invMultiplier[4][4] = {
	{0x0E,0x0B,0x0D,0x09},
	{0x09,0x0E,0x0B,0x0D},
	{0x0D,0x09,0x0E,0x0B},
	{0x0B,0x0D,0x09,0x0E}
...
void getInvSboxValues(unsigned char row[4]) {
	for (int i = 0; i < 4; i++) {
		unsigned char temp = row[i];
		int temp1 = (int)temp & 0x0f;
		int temp2 = (int)(temp >> 4) & 0x0f;
		//printf("%x\n", sbox[temp1][temp2]);
		row[i] = invSbox[temp2][temp1];
	}
}
unsigned char invHexMultiply(unsigned char aa, unsigned char bb) {
	int a = (int)aa & 0xff, b = (int)bb & 0xff;
	unsigned char temp;
	if (a == 9) {
		temp = mul9[b];
	}
...
void invMixColumns(unsigned char block[4][4]) {
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int j = 0; j < 4; j++) {
			...
			for (int k = 0; k < 4; k++) {
				printf(" %02x * %02x \n", invMultiplier[j][k], block[i][k]);
				temp[k] = invHexMultiply(invMultiplier[j][k], block[i][k]);
				printf(" tempk %02x\n",temp[k] );
			}
...
int main() {
	...
	printf("Place cipherText.txt or cipherImage.jpg  and cipherKey.txt files in parent directory \n\n");
	...
	if (option == '1') {
		...
		FILE *pt, *ck, *ct;
		ct = fopen("E:\\8th semester\\cs\\mid project\\projectSpace\\cipherText.txt", "r");
		pt = fopen("E:\\8th semester\\cs\\mid project\\projectSpace\\decryptedText.txt", "w");

		while (!readBlockFromFile(cipherText, ct)) {
			printf("ct");
			printBlock(cipherText);
			decrypt();
			writeBlockToFile(plainText, pt);
		}
		fclose(ct);
		fclose(pt);
	}
		...
		int temp;
		unsigned char temp1;
		pi = fopen("E:\\8th semester\\cs\\mid project\\projectSpace\\decryptedImage.jpg", "wb");
		ci = fopen("E:\\8th semester\\cs\\mid project\\projectSpace\\cipherImage.jpg", "rb");
		while (!readBlockFromImage(cipherText, ci))
		{
			decrypt();
			writeBlockToImage(plainText, pi);
		}
		fclose(pi);
		fclose(ci);
	}
	...
	int temp;
	unsigned char temp1;
	ct = fopen("E:\\8th semester\\cs\\mid project\\projectSpace\\temp.jpg", "rb");
	pt = fopen("E:\\8th semester\\cs\\mid project\\projectSpace\\temp1.jpg", "wb");
	while ((temp = fgetc(ct)) != EOF)
	{
		temp1 = temp & 0xFF;
		fputc(temp1, pt);
		printf("%d", temp1);
